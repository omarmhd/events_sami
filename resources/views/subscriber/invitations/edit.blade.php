            ...
            <form action="{{ route('invitations.update',$row->id) }}" method="POST">
                ...
                <div class="row g-4">
                    <div class="col-md-12">
                        ...
                        <div class="input-group-custom @error('event_id') has-error @enderror">
                            <i class="fas fa-calendar input-icon"></i>
                            <select class="form-control-styled @error('event_id') is-invalid-custom @enderror"
                                    id="event_id"
                                    name="event_id">
                                <option value="">Select event</option>
                                @foreach(($events ?? collect()) as $eventItem)
                                    <option value="{{ $eventItem->id }}" {{ old('event_id', $row->event_id) == $eventItem->id ? 'selected' : '' }}>
